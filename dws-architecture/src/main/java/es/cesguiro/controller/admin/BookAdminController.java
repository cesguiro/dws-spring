...
import es.cesguiro.domain.port.input.BookGetAllUseCase;
...
public class BookAdminController {
    ...
    public ResponseEntity<PaginatedResponse<BookCollection>> getAll(
        ...
        ListWithCount<BookCollectionDto> bookList = bookGetAllUseCase.execute(page - 1, pageSize);
    ...
    @PostMapping
    public ResponseEntity<Void> insert(@RequestBody BookInsertDto bookInsertDto) {
        bookInsertUseCase.execute(bookInsertDto);
        return new ResponseEntity<>(HttpStatus.CREATED);
    }
